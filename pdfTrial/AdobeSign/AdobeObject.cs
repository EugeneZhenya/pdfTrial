...
namespace pdfTrial.AdobeSign
{
    public class AdobeObject
    {
        ...
        public async Task<SigningUrlSet> GetAgreementSigningUrls(string agreementID)
        {
            //On null or empty agreement id, API is returning all agreements.
            if (string.IsNullOrWhiteSpace(agreementID))
            {
            ...
            string json = await API.GetRestJson(endpoint);
            return API.DeserializeJSon<SigningUrlSet>(json);
        }
        ...
        {
            var jsonContent = API.SerializeJSon(agreementCreationInfo);
